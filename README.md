...
- `/`: ツール一覧
- `/cellular-automaton/`: 任意のLife-likeルールを動かすセル・オートマトン
- `/holodori/`: ホロドリ用ツール一覧
...
各ツールの共有ボタンは、入力値や計算結果を含めず、現在の言語だけを付けたページURLを共有します。全ページが日本語と英語に対応しています。イベントPt調整ツールのv0.1は0ブーストでの完全一致だけを対象にしています。
...
同じページの日次ルートは、実際に成立した役と本日の受取済みコインから、30,000コイン以上を狙う停止候補を計算します。累計20,000コインちょうどで次ゲームを開始できなくなるため、20,000〜29,999コインで終了する候補は推奨しません。18,800〜19,900コインは、次の有効役を強制終了まで続ければ役の種類を問わず30,000コインへ届く範囲として扱います。

日次ルートは、実現した役から次の停止点を選ぶ実用的な規則です。将来の手札、BET残高、試行時間まで含めた日全体の確率的最適化ではありません。受取履歴はブラウザ内だけに保存し、毎日5:00（日本時間）に新しい記録へ切り替えます。
...
- 静的HTMLと未対応言語のフォールバックは英語です。URL、保存済み設定、ブラウザ言語で日本語が選ばれた場合は日本語へ切り替わります。
